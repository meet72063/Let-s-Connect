...
const EditPost = () => {
    ...
    const {title,content,author,createdAt,_id} = posts.find((post)=>post._id===id)
...
const onSubmitHandler=async()=>{
  try {
    await axios.patch(`http://localhost:4000/post/${_id}`, postData,{withCredentials:true})
    setPostData({title:'',content:'',author:''})
    navigate(-1)
  } catch (error) {
    setErr(error.response.msg||'Oops something went wrong!')
  }
}
...
// 
  ...
  return (
   ...
   <div className="relative w-[32rem] border-2 border-gray-400 rounded-2xl p-3 flex flex-col pt-5 gap-6 ml-10">
    ...
    <div className="w-full flex justify-between py-1.5">
      ...
      <div className="flex gap-2 ml-80">
        ...
        <Button size="sm" className="rounded-md" type='submit' onClick={onSubmitHandler}>Save</Button>
      </div>
    </div>
  </div>
</div>
